...
extension UIViewController {
    ...
    func noticeOnlyText(_ text: String, autoClear: Bool = true, autoClearTime: Int = 3) {
        
        if text == "1000"
        {
            let alert = UIAlertController.init(title: "网络请求错误", message: "网络请求出现问题，请检查网络设置", preferredStyle: .alert)
            let cancer = UIAlertAction.init(title: "取消", style: .cancel, handler: nil)
            let defaults = UIAlertAction.init(title: "查看解决方案", style: .destructive, handler: { (UIAlertAction) in
                let questionView = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "QuestionViewController") as! QuestionViewController
...
class SwiftNotice: NSObject {
    ...
    static func noticeOnSatusBar(_ text: String, autoClear: Bool, autoClearTime: Int) -> UILabel {
        ...
        if autoClear {
            ...
            self.perform(selector, with: window, afterDelay: Foundation.TimeInterval(autoClearTime))
        }
    ...
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        let frame = CGRect(x: (UIScreen.main.bounds.size.width - 78)/2.0, y: (screenHeight - 78)/2.0, width: 78, height: 78)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        ...
        if imageNames.count > 0 {
            if imageNames.count > timerTimes {
                ...
                let time = timeInterval*Int(NSEC_PER_SEC)
                  timer?.scheduleRepeating(deadline: .now(), interval: .seconds(time), leeway: .seconds(0))
//                timer?.sche(start: DispatchTime.now(), interval: UInt64(timeInterval) * NSEC_PER_MSEC, leeway: 0)
