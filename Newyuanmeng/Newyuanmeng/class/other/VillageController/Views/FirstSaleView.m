...
@interface FirstSaleView()<UITableViewDelegate,UITableViewDataSource>
...
- (void)setUser_id:(NSInteger)user_id withToken:(NSString *)token withDistrict_id:(NSString *)district_id{
    ...
    [MySDKHelper postAsyncWithURL:@"/v1/get_district_sale_record" withParamBodyKey:keys withParamBodyValue:values needToken:_token postSucceed:^(NSDictionary *result) {
        NSLog(@"我是数据%@",result);
        //解析
        //容错
        ...
        for (NSDictionary *dic in result[@"content"][@"data"]) {
            SaleRecordModel *model = [[SaleRecordModel alloc] initWithDictionary:dic error:nil];
...
- (FirstSaleViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    FirstSaleViewCell *cell  = [tableView dequeueReusableCellWithIdentifier:@"cellID"];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"FirstSaleViewCell" owner:nil options:nil] firstObject];
