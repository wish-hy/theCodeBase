...
@property (nonatomic, assign) NSInteger temuIndex;
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [line mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (tableView.tag == TableViewTag)
        {
            ...
            if (indexPath.row == self.temuSelectIndex && ( self.tmpOuterArray.count == 0 || self.tmpOuterArray == nil)) {
                 lbl.textColor = [UIColor colorWithRed:255/255.0 green:117/255.0 blue:13/255.0 alpha:1];
                line.backgroundColor = [UIColor colorWithRed:255/255.0 green:117/255.0 blue:13/255.0 alpha:1];
            }else if (indexPath.row > 0 && self.secondArray.count > 0){
                line.hidden = YES;
            }
        }
        ...
        else if (tableView.tag == TableViewTag + 1)
        {
            ...
            [cell.contentView addSubview:lbl];
            if (indexPath.row == self.selectIndex && self.temuIndex == self.temuSelectIndex) {
                lbl.textColor = [UIColor colorWithRed:255/255.0 green:117/255.0 blue:13/255.0 alpha:1];
                line.backgroundColor = [UIColor colorWithRed:255/255.0 green:117/255.0 blue:13/255.0 alpha:1];
            }
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(self.isTowTable == 1) {
        if (tableView.tag == TableViewTag) {
            self.secondArray = self.tmpOuterArray[indexPath.row];
            //刷新tableView高度
            [self layoutTableViewHeight];
            [self.tableView_1 reloadData];
            [self.tableView_2 reloadData];
            _temuIndex = indexPath.row;
        } else if (tableView.tag == TableViewTag + 1) {
            self.selectIndex = indexPath.row;
            if(self.delegate && [self.delegate respondsToSelector:@selector(tableViewDidSelect:obj:)]){
                [self.delegate tableViewDidSelect:[self.tmpOuterArray indexOfObject:self.secondArray] obj:[@{ @"name" : self.secondArray[indexPath.row] , @"row" : [NSString stringWithFormat:@"%ld",(long)indexPath.row]} mutableCopy]];
            }
            [self dissmissChangeBtnSelect];
        }
    }else {
        [self.tableView_1 reloadData];
        self.temuIndex = indexPath.row;
        if(self.delegate && [self.delegate respondsToSelector:@selector(tableViewDidSelect:obj:)]){
